...
**S2** — 1) two tenants on one host · 2) spoof-proof resolution · 3) private origin (403 without edge auth) · 4) tenant isolation (A can't read B; no-tenant query throws; cross-tenant path → 404) · 5) data-driven routing (new route = a DB row, no rebuild).
