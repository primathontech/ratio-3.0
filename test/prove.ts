...
(async () => {
  ...
  check(
    ...
    acme.body.includes('Acme') &&
      beta.body.includes('Beta') &&
      acme.headers['x-tenant'] === 't_acme' &&
      beta.headers['x-tenant'] === 't_beta',
  ...
  check(
    ...
    `served ${spoof.headers['x-tenant']}`
  );

  const direct = await get(ORIGIN, '/', { 'x-ratio-tenant': 't_acme' });
  check(
    '3. Origin is private (direct request w/o edge auth -> 403)',
    direct.status === 403,
  ...
  check(
    '7. Unknown host -> park page (404)',
    unknown.status === 404 && /Store not found/.test(unknown.body)
  );
