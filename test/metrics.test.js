...
// cardinality (per-tenant labels can explode → cap + aggregate the tail).
// Pure unit, deterministic. Written test-first.
...
test('bounds tenant-label cardinality (overflow aggregates into _other)', () => {
  ...
  m.inc('render', { tenant: 't3' }); // 3rd distinct label -> _other
  m.inc('render', { tenant: 't4' });
  const snap = m.snapshot().render;
  assert.ok('t1' in snap && 't2' in snap);
  assert.strictEqual(snap._other, 2); // t3 + t4 folded in
  assert.strictEqual(m.total('render'), 4); // total still exact
});
