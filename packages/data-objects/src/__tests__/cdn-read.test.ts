...
test('immutable versions/ key is read from the CDN, not the inner store', async () => {
  let url = '';
  ...
  const bytes = await cdn.get('themes/t1/versions/compiled/abc.gz');
  assert.equal(new TextDecoder().decode(bytes!), 'BUNDLE');
  assert.equal(url, 'https://cdn.example/themes/t1/versions/compiled/abc.gz');
...
test('CDN non-2xx falls back to the inner (authoritative) store', async () => {
  ...
  assert.deepEqual(inner.calls, ['get:themes/t1/versions/compiled/abc.gz']);
});
...
test('CDN network error falls back to the inner store', async () => {
  ...
  await cdn.get('themes/t1/versions/compiled/abc.gz');
  assert.deepEqual(inner.calls, ['get:themes/t1/versions/compiled/abc.gz']);
});
...
test('writes/head/delete always go to the inner store, never the CDN', async () => {
  ...
  await cdn.put('themes/t1/versions/compiled/x.gz', enc('x'));
  await cdn.head('themes/t1/versions/compiled/x.gz');
  await cdn.delete('themes/t1/versions/compiled/x.gz');
  assert.deepEqual(inner.calls, [
    'put:themes/t1/versions/compiled/x.gz',
    'head:themes/t1/versions/compiled/x.gz',
    'delete:themes/t1/versions/compiled/x.gz',
  ]);
});
...
test('keyPrefix is applied to the CDN URL and a trailing slash on the base is trimmed', async () => {
  ...
  await cdn.get('themes/t1/versions/compiled/abc.gz');
  assert.equal(url, 'https://cdn.example/ns/themes/t1/versions/compiled/abc.gz');
});
