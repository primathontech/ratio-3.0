...
test('immutable versions/ key is read from the CDN, not the inner store', async () => {
  ...
  const bytes = await cdn.get('versions/compiled/abc.gz');
  assert.equal(new TextDecoder().decode(bytes!), 'BUNDLE');
  assert.equal(url, 'https://cdn.example/versions/compiled/abc.gz');
...
test('CDN non-2xx falls back to the inner (authoritative) store', async () => {
  ...
  assert.deepEqual(inner.calls, ['get:versions/compiled/abc.gz']);
});
...
test('CDN network error falls back to the inner store', async () => {
  ...
  await cdn.get('versions/compiled/abc.gz');
  assert.deepEqual(inner.calls, ['get:versions/compiled/abc.gz']);
});
...
test('writes/head/delete always go to the inner store, never the CDN', async () => {
  ...
  await cdn.put('versions/compiled/x.gz', enc('x'));
  await cdn.head('versions/compiled/x.gz');
  await cdn.delete('versions/compiled/x.gz');
  assert.deepEqual(inner.calls, [
    'put:versions/compiled/x.gz',
    'head:versions/compiled/x.gz',
    'delete:versions/compiled/x.gz',
  ]);
});
...
test('keyPrefix is applied to the CDN URL and a trailing slash on the base is trimmed', async () => {
  ...
  await cdn.get('versions/compiled/abc.gz');
  assert.equal(url, 'https://cdn.example/ns/versions/compiled/abc.gz');
});
